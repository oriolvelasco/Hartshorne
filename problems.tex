...
\begin{sol}
	...
	%  Given an element $\overline{(V,t)} \in \varinjlim_{V \supseteq f(U)} \sG(V)$ we denote its image $\varphi(U)(\overline{V,t})$. $\varphi'$ induces then an application $\varphi'_*: f_*f^{-1}\sG \to f_* \sF$ that maps each $\overline{(V,t)} \in \varinjlim_{W \supseteq f(f^{-1}(V))} \sG(W)$ to $\varphi(f^{-1}(V))$


	%it induces a morphism $\phi_{*}: f_{*}f^{-1}\sG \to f_{*}\sF$. Then, $\phi_{*} \circ \psi: \sG \to f_{*}\sF$. Similarly, given $\phi: \sG \to f_{*}\sF$ it induces a morphism $\phi': f^{-1}\sG \to f^{-1}f_{*}\sF$. Then, $\varphi^{+} \circ \phi': f^{-1}\sG \to \sF$. We have to check that this correspondence is bijective. We will prove that if we make one proceding and then the other one, we obtain the initial morphism. It is enough to check that this happens when we consider morphisms of presheaves. 
\end{sol}
